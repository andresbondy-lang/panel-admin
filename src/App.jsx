...
const CLUBS = [
  { id:"highera",    name:"High Era",   color:"#7ec850", accent:"#4a9a20", shopKey:"highera-shopping-list",    logKey:"highera-feed-log",    trazKey:"highera-trazabilidad" },
  { id:"weedmaster", name:"Weedmaster", color:"#70c8c8", accent:"#209a9a", shopKey:"weedmaster-shopping-list", logKey:"weedmaster-feed-log", trazKey:"weedmaster-trazabilidad" },
];
...
export default function App() {
  ...
  const loadAll = async () => {
    ...
    for (const club of CLUBS) {
      try { const r=await window.storage.get(club.shopKey,true); if(r?.value) result[club.id].shop=safeParse(r.value,[]); } catch {}
      try { const r=await window.storage.get(club.logKey,true);  if(r?.value) result[club.id].log=safeParse(r.value,[]);  } catch {}
      try { const r=await window.storage.get(club.trazKey,true); if(r?.value){ const p=safeParse(r.value,null); if(p) result[club.id].traz=p; } } catch {}
    }
  ...
  const toggleDone = async (clubId, itemId) => {
    ...
    try { await window.storage.set(club.shopKey,JSON.stringify(updated),true); } catch {}
  };
              ...
              return (
                <div key={c.id}>
                  <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"10px"}}>
                    ...
                    {done.length>0&&<button onClick={()=>clearDone(c.id)} style={{marginLeft:"auto",fontSize:"12px",color:"#c87070",background:"transparent",border:"none",cursor:"pointer",fontWeight:600}}>Limpiar comprados</button>}
                  </div>

                  {pending.length===0&&done.length===0&&<div style={{fontSize:"13px",color:"#3a5a3a",padding:"6px 0"}}>Sin productos pendientes ✓</div>}
                ...
                <div key={`${entry.clubId}-${entry.id}`} style={S.card(entry.color)}>
                  <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:"9px"}}>
                    <div>
                      ...
                      <div style={{fontSize:"12px",color:"#3a5a7a",marginTop:"3px"}}>{entry.date} · {entry.author}</div>
                      {entry.stage&&<span style={{fontSize:"11px",background:"#1a2a3a",color:"#5a8aaa",padding:"2px 8px",borderRadius:"20px",marginTop:"4px",display:"inline-block"}}>{entry.stage}</span>}
                    </div>
                  </div>
                  {entry.nutrients?.length>0&&<div style={{marginBottom:"7px"}}><div style={{fontSize:"11px",color:"#3a5a7a",marginBottom:"5px"}}>NUTRIENTES</div><div style={{display:"flex",flexWrap:"wrap",gap:"5px"}}>{entry.nutrients.map((n,i)=><span key={i} style={{background:"#1a2a3a",border:"1px solid #2a4a6a",color:"#8ab8e8",padding:"2px 9px",borderRadius:"20px",fontSize:"12px"}}>{n.name}{n.dose?` · ${n.dose}`:""}</span>)}</div></div>}
                  ...
                  {entry.treatments?.length>0&&<div style={{marginBottom:"7px"}}><div style={{fontSize:"11px",color:"#7a4ab8",marginBottom:"5px"}}>TRATAMIENTOS</div><div style={{display:"flex",flexWrap:"wrap",gap:"5px"}}>{entry.treatments.map((t,i)=><span key={i} style={{background:"#2a1e3a",border:"1px solid #5a3a9a",color:"#b07ee8",padding:"2px 9px",borderRadius:"20px",fontSize:"12px"}}>{t.name}{t.dose?` · ${t.dose}`:""}</span>)}</div></div>}
                    ...
                    return (
                      <div key={c} style={{background:"#0f1828",border:`1px solid ${club.color}22`,borderRadius:"9px",overflow:"hidden",marginBottom:"7px"}}>
                        <button onClick={()=>setExpanded(e=>({...e,[key]:!e[key]}))}
                          style={{width:"100%",background:"transparent",border:"none",cursor:"pointer",padding:"11px 13px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
